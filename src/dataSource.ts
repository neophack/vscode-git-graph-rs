...
export class DataSource extends Disposable {
	...
	/**
	 * Edit the commit message of any local commit that has not been pushed to a remote.
	 *
	 ...
	 * @param commitHash The commit hash to edit the message of.
	...
	public async editCommitMessage(repo: string, commitHash: string, message: string): Promise<ErrorInfo> {
		...
		try {
			...
			// Editing a commit any remote already contains would rewrite published history
	 ...
	 * @param extraEnv Additional environment variables layered over the process and askpass environments.
	 */
